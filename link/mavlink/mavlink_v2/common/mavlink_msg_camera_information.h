...
typedef struct __mavlink_camera_information_t {
    uint32_t time_boot_ms;           /*< [毫秒] 时间戳（系统启动以来的时间）*/
    uint32_t firmware_version;       /*< 相机固件版本，编码格式为：`(Dev & 0xff) << 24 + (Patch & 0xff) << 16 + (Minor & 0xff) << 8 + (Major & 0xff)`。若未知则填 0*/
    float focal_length;              /*< [毫米] 焦距。若未知则填 NaN*/
    float sensor_size_h;             /*< [毫米] 图像传感器水平尺寸。若未知则填 NaN*/
    float sensor_size_v;             /*< [毫米] 图像传感器垂直尺寸。若未知则填 NaN*/
    uint32_t flags;                  /*< 相机能力标志的位图*/
    uint16_t resolution_h;           /*< [像素] 水平图像分辨率。若未知则填 0*/
    uint16_t resolution_v;           /*< [像素] 垂直图像分辨率。若未知则填 0*/
    uint16_t cam_definition_version;/*< 相机定义版本（迭代次数）。若未知则填 0*/
    uint8_t vendor_name[32];         /*< 相机供应商名称*/
    uint8_t model_name[32];          /*< 相机型号名称*/
    uint8_t lens_id;                 /*< 预留用于镜头 ID。若未知则填 0*/
    char cam_definition_uri[140];    /*< 相机定义文件 URI（如有，否则仅支持基础功能）。允许 HTTP（http://）和 MAVLink FTP（mavlinkftp://）格式的 URI（任何实现相机协议的 GCS 必须支持两者）。定义文件可能为 xz 压缩格式，以文件扩展名 .xml.xz 标识（实现协议的 GCS 必须支持解压）。字符串需以零结尾。若未知则使用空字符串*/
    uint8_t gimbal_device_id;        /*< 与此相机关联的云台 ID。对于非 MAVLink 云台，值为 1-6；若未关联云台则填 0*/
    uint8_t camera_device_id;        /*< 连接到自动驾驶仪的非 MAVLink 相机 ID（1-6）。若该组件为 MAVLink 相机（拥有自己的组件 ID）则填 0*/
