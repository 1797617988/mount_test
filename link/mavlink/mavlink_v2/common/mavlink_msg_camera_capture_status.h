...
typedef struct __mavlink_camera_capture_status_t {
 ...
 float image_interval;      /*< [秒] 图像捕获间隔 */
 uint32_t recording_time_ms; /*< [毫秒] 自开始录制以来经过的时间（0：不支持/不可用）。
                               地面站（GCS）应计算录制时间，并使用此字段的非零值来纠正任何差异。*/
 float available_capacity;   /*< [MiB] 可用存储容量。*/
 uint8_t image_status;      /*< 图像捕获的当前状态（0：空闲，1：捕获中，
                               2：已设置间隔但空闲，3：已设置间隔且捕获中）*/
 uint8_t video_status;      /*< 视频捕获的当前状态（0：空闲，1：捕获中）*/
 int32_t image_count;       /*< 捕获的图像总数（'永久'，或直到使用MAV_CMD_STORAGE_FORMAT重置）。*/
 uint8_t camera_device_id;  /*< 连接到自动驾驶仪的非MAVLink相机的摄像头ID（1-6）。
                               如果组件是MAVLink相机（有自己的组件ID），则为0。*/
