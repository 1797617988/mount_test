...
typedef struct __mavlink_video_stream_status_t {
 float framerate;         /*< [Hz] 帧率 */
 uint32_t bitrate;        /*< [bits/s] 比特率（码率） */
 uint16_t flags;          /*< 流状态标志位图 */
 uint16_t resolution_h;    /*< [像素] 水平分辨率 */
 uint16_t resolution_v;    /*< [像素] 垂直分辨率 */
 uint16_t rotation;       /*< [度] 视频图像顺时针旋转角度 */
 uint16_t hfov;           /*< [度] 水平视野角度 */
 uint8_t stream_id;       /*< 视频流ID（第一个流为1，第二个为2，以此类推） */
