/** @file
 *  @brief MAVLink 通用消息定义（common.xml）
 *  @see http://mavlink.org
 *  本文件定义了 MAVLink 协议的通用消息和枚举类型。
 *  所有英文注释已翻译为中文，便于理解和维护。
 */
#pragma once
...
#ifndef MAVLINK_H
    #error 错误的包含顺序：MAVLINK_COMMON.H 不能直接使用。请包含同目录下的 mavlink.h，或者手动设置 MAVLINK.H 中的所有定义，包括 #define MAVLINK_H 调用。
#endif

#define MAVLINK_COMMON_XML_HASH -8393734385304460379
...
/** @brief 用于在高延迟遥测中报告故障情况的标志位。 */
#ifndef HAVE_ENUM_HL_FAILURE_FLAG
#define HAVE_ENUM_HL_FAILURE_FLAG
typedef enum HL_FAILURE_FLAG
{
   HL_FAILURE_FLAG_GPS=1, /* GPS故障。 | */
   HL_FAILURE_FLAG_DIFFERENTIAL_PRESSURE=2, /* 差压传感器故障。 | */
   HL_FAILURE_FLAG_ABSOLUTE_PRESSURE=4, /* 绝对压力传感器故障。 | */
   HL_FAILURE_FLAG_3D_ACCEL=8, /* 加速度计传感器故障。 | */
   HL_FAILURE_FLAG_3D_GYRO=16, /* 陀螺仪传感器故障。 | */
   HL_FAILURE_FLAG_3D_MAG=32, /* 磁力计传感器故障。 | */
   HL_FAILURE_FLAG_TERRAIN=64, /* 地形子系统故障。 | */
   HL_FAILURE_FLAG_BATTERY=128, /* 电池故障/电池严重不足。 | */
   HL_FAILURE_FLAG_RC_RECEIVER=256, /* 遥控接收器故障/无遥控连接。 | */
   HL_FAILURE_FLAG_OFFBOARD_LINK=512, /* 离线链路故障。 | */
   HL_FAILURE_FLAG_ENGINE=1024, /* 引擎故障。 | */
   HL_FAILURE_FLAG_GEOFENCE=2048, /* 地理围栏违规。 | */
   HL_FAILURE_FLAG_ESTIMATOR=4096, /* 估计器故障，例如测量拒绝或大方差。 | */
   HL_FAILURE_FLAG_MISSION=8192, /* 任务故障。 | */
   HL_FAILURE_FLAG_ENUM_END=8193, /*  | */
} HL_FAILURE_FLAG;
#endif

/** @brief 可在 MAV_CMD_OVERRIDE_GOTO 中指定的用于覆盖任务执行的动作。 */
...
/** @brief 预定义的 MAV_MODE_FLAG 组合值。用于简化设置模式时的标志使用。注意：所有模式下都启用手动输入作为安全保护。 */
...
/** @brief 该枚举用于编码 SYS_STATUS 消息中报告的各类传感器状态。 */
...
/** @brief 该枚举用于编码 SYS_STATUS 消息扩展字段中报告的传感器状态。 */
...
/** @brief MAVLink 坐标系定义。并非所有命令、消息或飞行器都支持所有坐标系。

         全局坐标系命名约定：
         - "GLOBAL"：WGS84 全球坐标系，默认高度为海平面（MSL）以上。
            可选修饰符：
            - "RELATIVE_ALT"：高度相对于飞行器的 Home 点。
            - "TERRAIN_ALT"：高度相对于地面。
            - "INT"：经纬度乘以 1E7 进行缩放。

         局部坐标系命名约定：
         - "LOCAL"：原点固定在地球上，通常为定位估算器（EKF）原点。
         - "BODY"：原点随飞行器移动。注意，“BODY”不代表坐标轴与飞行器姿态对齐。
         - "OFFSET"：已废弃，等同于“BODY”，不建议新项目使用。

         部分废弃坐标系不遵循上述约定（如 MAV_FRAME_BODY_NED 和 MAV_FRAME_BODY_OFFSET_NED）。
   */
#ifndef HAVE_ENUM_MAV_FRAME
#define HAVE_ENUM_MAV_FRAME
typedef enum MAV_FRAME
{
   MAV_FRAME_GLOBAL=0, /* 全球坐标系（WGS84），高度相对于海平面（MSL）。 */
   MAV_FRAME_LOCAL_NED=1, /* NED局部切线坐标系（x:北，y:东，z:下），原点固定在地球上。 */
   MAV_FRAME_MISSION=2, /* 非坐标系，仅表示任务命令。 */
   MAV_FRAME_GLOBAL_RELATIVE_ALT=3, /* 全球坐标系（WGS84），高度相对于Home点。 */
   MAV_FRAME_LOCAL_ENU=4, /* ENU局部切线坐标系（x:东，y:北，z:上），原点固定在地球上。 */
   MAV_FRAME_GLOBAL_INT=5, /* 全球坐标系（WGS84，缩放），高度相对于海平面（MSL）。 */
   MAV_FRAME_GLOBAL_RELATIVE_ALT_INT=6, /* 全球坐标系（WGS84，缩放），高度相对于Home点。 */
   MAV_FRAME_LOCAL_OFFSET_NED=7, /* NED局部切线坐标系，原点随飞行器移动。 */
   MAV_FRAME_BODY_NED=8, /* 用于位置时同MAV_FRAME_LOCAL_NED，用于速度/加速度时同MAV_FRAME_BODY_FRD。 */
   MAV_FRAME_BODY_OFFSET_NED=9, /* 等同于MAV_FRAME_BODY_FRD。 */
   MAV_FRAME_GLOBAL_TERRAIN_ALT=10, /* 全球坐标系（WGS84），高度为地面高度（AGL）。 */
   MAV_FRAME_GLOBAL_TERRAIN_ALT_INT=11, /* 全球坐标系（WGS84，缩放），高度为地面高度（AGL）。 */
   MAV_FRAME_BODY_FRD=12, /* FRD局部坐标系，随飞行器姿态对齐（x:前，y:右，z:下），原点随飞行器移动。 */
   MAV_FRAME_RESERVED_13=13, /* MAV_FRAME_BODY_FLU - 机体固定坐标系，Z轴向上（x:前，y:左，z:上）。 */
   MAV_FRAME_RESERVED_14=14, /* MAV_FRAME_MOCAP_NED - 运动捕捉系统提供的NED坐标系，Z轴向下。 */
   MAV_FRAME_RESERVED_15=15, /* MAV_FRAME_MOCAP_ENU - 运动捕捉系统提供的ENU坐标系，Z轴向上。 */
   MAV_FRAME_RESERVED_16=16, /* MAV_FRAME_VISION_NED - 视觉估算系统提供的NED坐标系，Z轴向下。 */
   MAV_FRAME_RESERVED_17=17, /* MAV_FRAME_VISION_ENU - 视觉估算系统提供的ENU坐标系，Z轴向上。 */
   MAV_FRAME_RESERVED_18=18, /* MAV_FRAME_ESTIM_NED - 飞行器板载估算器提供的NED坐标系，Z轴向下。 */
   MAV_FRAME_RESERVED_19=19, /* MAV_FRAME_ESTIM_ENU - 飞行器板载估算器提供的ENU坐标系，Z轴向上。 */
   MAV_FRAME_LOCAL_FRD=20, /* FRD局部切线坐标系（x:前，y:右，z:下），原点固定在地球上，前轴与飞行器前方对齐。 */
   MAV_FRAME_LOCAL_FLU=21, /* FLU局部切线坐标系（x:前，y:左，z:上），原点固定在地球上，前轴与飞行器前方对齐。 */
   MAV_FRAME_ENUM_END=22, /* 枚举结束标志。 */
} MAV_FRAME;
#endif
